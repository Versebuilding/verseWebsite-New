...
export default function PolyWalk () {
  ...
  return (
    <section className='bg-[#0b0b17] text-white py-18'>
      {/*  pills */}
<div className="mb-10">
  <div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap sm:justify-center sm:gap-3 text-base font-medium">
    {polyContent.map((p, i) => {
      const activeState = active === i
      return (
        <button
          key={p.label}
          onClick={() => setActive(i)}
          ...
          className={[
            "px-4 py-3 rounded-2xl transition-all duration-300 border text-sm sm:text-base",
            activeState
              ? "bg-gradient-to-r from-indigo-500 to-fuchsia-500 border-transparent shadow-lg"
              : "bg-[#191929] hover:bg-[#23233a] border-white/10"
          ].join(" ")}
        >
          {p.label}
        </button>
      ...
      {/* panel */}
<div className="max-w-[88rem] lg:px-8 mx-auto flex flex-col lg:flex-row gap-12 lg:gap-18 items-center">
        {/* image */}
  ...
  <div className="w-full lg:w-1/2 space-y-4 text-center lg:text-left">
    ...
    <p className="text-base md:text-lg text-gray-300 leading-relaxed">{blurb}</p>
        </div>
      </div>
    </section>
  )
